    ...
    if ($.JdFarmProdName != "") {
        if ($.JdtreeEnergy != 0) {
            if ($.treeState === 2 || $.treeState === 3) {
                allReceiveMessage += `【狗东帐号：${$.nickName || $.UserName}】(东东农场)${$.JdFarmProdName} 可以兑换了\n`;
            }
        }
    }
    if ($.DdFactoryReceive) {
        allReceiveMessage += `【狗东帐号：${$.nickName || $.UserName}】(东东工厂)${$.DdFactoryReceive}可以兑换了\n`;
    }
    if ($.jxFactoryReceive) {
        allReceiveMessage += `【狗东帐号：${$.nickName || $.UserName}】(京喜工厂)${$.jxFactoryReceive}可以兑换了 \n`;
    }
    const initPetTownRes = await PetRequest('initPetTown');
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
        $.petInfo = initPetTownRes.result;
        if ($.petInfo.petStatus === 5) {
            allReceiveMessage += `【狗东帐号：${$.nickName || $.UserName}】(东东萌宠)${$.petInfo.goodsInfo.goodsName}可以兑换了!\n`;
        }
    }
}
